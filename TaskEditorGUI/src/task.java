...
public class TaskEditorApplication extends JFrame {
    ...
    private void addTask() {
        ...
        // Check if the task deadline is today and show notification immediately
        if (newTask.getDeadline() != null && newTask.getDeadline().equals(LocalDate.now())) {
            showNotification("Task Deadline Alert", "Task: " + newTask.getName() + " is due today!");
        }
    }
    ...
    private void showNotification(String title, String message) {
        try {
            ...
            Image image = Toolkit.getDefaultToolkit().createImage("path/to/icon.png"); // Add the correct icon path here
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            // For testing, create a task immediately with today's date
            Task testTask = new Task(1, "Test Task", "Test Description", LocalDate.now(), false);
            app.tasks.add(testTask);
            app.tableModel.addRow(new Object[]{testTask.getId(), testTask.getName(), testTask.getDescription(), testTask.getDeadline(), testTask.isCompleted()});

            // Trigger notification for the test task immediately
            app.showNotification("Test Task Deadline Alert", "Task: " + testTask.getName() + " is due today!");
        });
    }
}
